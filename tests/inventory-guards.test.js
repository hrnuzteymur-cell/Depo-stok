...
const healthyAudit = guards.auditState(baseState());
assert.equal(healthyAudit.ok, true);
assert.equal(healthyAudit.issues.length, 0);
assert.equal(healthyAudit.warnings.length, 0);
assert.equal(healthyAudit.summary.totalQty, 10);
assert.equal(healthyAudit.summary.products, 2);
assert.equal(healthyAudit.summary.stockRows, 1);
...
assert.equal(guards.auditState(duplicateProduct).issues.some(i => i.code === 'DUPLICATE_PRODUCT'), true);
...
assert.equal(guards.auditState(unknownProductStock).issues.some(i => i.code === 'UNKNOWN_PRODUCT_STOCK'), true);
...
assert.equal(guards.auditState(unknownShelfStock).issues.some(i => i.code === 'UNKNOWN_SHELF_STOCK'), true);

const negativeStock = baseState();
negativeStock.stock[0].qty = -2;
assert.equal(guards.validateState(negativeStock).ok, false);
assert.equal(guards.auditState(negativeStock).issues.some(i => i.code === 'INVALID_QTY'), true);

const duplicateStockRow = baseState();
duplicateStockRow.stock.push({ barcode: 'P1', location: 'A1', qty: 2 });
const duplicateStockAudit = guards.auditState(duplicateStockRow);
assert.equal(duplicateStockAudit.ok, true);
assert.equal(duplicateStockAudit.warnings.some(i => i.code === 'DUPLICATE_STOCK_ROW'), true);
assert.equal(duplicateStockAudit.summary.totalQty, 12);

const duplicateShelf = baseState();
duplicateShelf.shelves.push('A1');
const duplicateShelfAudit = guards.auditState(duplicateShelf);
assert.equal(duplicateShelfAudit.ok, true);
assert.equal(duplicateShelfAudit.warnings.some(i => i.code === 'DUPLICATE_SHELF'), true);
...
console.log('Inventory guard and consistency tests passed');
